  ...
  return (
    <div className="fixed bottom-4 right-4 z-40 flex flex-col gap-3 md:bottom-6 md:right-6">
      {BUTTONS.map(({ label, href, Icon, cls }) => (
        <motion.a key={label} href={href} target="_blank" rel="noopener noreferrer" aria-label={label} whileHover={{ scale: 1.12 }} whileTap={{ scale: 0.9 }} className={`relative grid size-14 place-items-center rounded-full text-white shadow-xl ring-2 ring-white/70 md:size-16 ${cls}`}>
          <Icon size={34} weight={label === "Instagram" ? "bold" : "fill"} />
        </motion.a>
      ))}
    </div>
  );
